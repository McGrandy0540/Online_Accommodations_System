...
// Initialize variables with default values matching database schema
$errors = [];
$success = '';
$userData = [
    'username' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_user'])) {
    ...
    $userData['status'] = trim($_POST['status'] ?? 'student');
    $userData['sex'] = trim($_POST['sex'] ?? 'male');
    $userData['location'] = trim($_POST['location'] ?? '');
    $userData['phone_number'] = trim($_POST['phone_number'] ?? '');
    $userData['payment_method'] = trim($_POST['payment_method'] ?? 'cash');
    ...
    if (empty($userData['email'])) {
        ...
        $stmt = $db->prepare("SELECT id FROM users WHERE email = ? AND deleted = 0");
    ...
    } elseif (strlen($userData['pwd']) < 8) {
        $errors['pwd'] = 'Password must be at least 8 characters';
    }

    // Validate status against database ENUM values
    $allowedStatuses = ['student', 'property_owner', 'admin'];
    if (!in_array($userData['status'], $allowedStatuses)) {
    ...
    if (empty($errors)) {
        try {
            ...
            // Hash password
            $hashedPassword = password_hash($userData['pwd'], PASSWORD_DEFAULT);
            
            $stmt = $db->prepare("INSERT INTO users 
                                (username, email, pwd, status, sex, location, phone_number, 
                                 payment_method, email_notifications, sms_notifications)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
// Handle user listing with pagination and filtering
$perPage = 10;
                    ...
                    <div class="form-col">
                        <div class="form-group">
                            ...
                            <select id="status" name="status" class="form-select" required>
                                <option value="student" <?php echo $userData['status'] === 'student' ? 'selected' : ''; ?>>Student</option>
                                <option value="property_owner" <?php echo $userData['status'] === 'property_owner' ? 'selected' : ''; ?>>Property Owner</option>
                                <option value="admin" <?php echo $userData['status'] === 'admin' ? 'selected' : ''; ?>>Admin</option>
                            </select>
                        </div>
                    ...
                    <div class="form-col">
                        ...
                        <div class="form-group">
                            ...
                            <select id="payment_method" name="payment_method" class="form-select">
                                <option value="cash" <?php echo $userData['payment_method'] === 'cash' ? 'selected' : ''; ?>>Cash</option>
                                <option value="mobile_money" <?php echo $userData['payment_method'] === 'mobile_money' ? 'selected' : ''; ?>>Mobile Money</option>
                                <option value="credit_card" <?php echo $userData['payment_method'] === 'credit_card' ? 'selected' : ''; ?>>Credit Card</option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input type="checkbox" id="email_notifications" name="email_notifications" 
                                       class="form-check-input" value="1" <?php echo $userData['email_notifications'] ? 'checked' : ''; ?>>
                                <label for="email_notifications">Email Notifications</label>
                            </div>
                            <div class="form-check">
                                <input type="checkbox" id="sms_notifications" name="sms_notifications" 
                                       class="form-check-input" value="1" <?php echo $userData['sms_notifications'] ? 'checked' : ''; ?>>
